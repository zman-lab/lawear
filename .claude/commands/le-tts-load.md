...
### Step 3.4. 사용자 두문자 정리 로드 (과목별, 신규)

> **시각 인덱스용**. TTS 본문에 두문자 그대로 사용 X — 풀이형 키워드로 변환 필수.
> PDF 강조 매칭 + 부장판사 QA 풀이형 키워드 검증용.

| 과목 | 경로 | 변환 시 |
|------|------|---------|
| 민사소송법 | `/Users/nhn/Documents/암기노트/민소/두문자_정리.md` | 민소 미케·모고 변환 시 Read |
| 민법 | `/Users/nhn/Documents/암기노트/민법/두문자_정리.md` | 민법 변환 시 Read (+ `26_민법_미케.txt` 정답지 같이) |
| 형법·형소·부등 | 추후 추가 예정 | - |

활용 룰:
- 두문자에 있는 표현 = R-09 회색지대 통과 (사용자가 시험에 쓰는 것)
- Lv.4 본문에 "위,발,간" 같은 두문자 X → "관할위반 / 발견시 조치 / 간과판결" 풀어쓰기
- 부장판사 QA 7번째 카테고리: "두문자 풀이형 핵심 키워드 누락 여부"
