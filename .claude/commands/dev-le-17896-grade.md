...
#### 2.2.5 Phase 0 — 기계 매칭 (조문·두문자·키워드 사전 추출, 토큰↓·일관성↑)

9기준 잠정 채점(2.3) 전, **기계적으로 추출 가능한 매칭**을 사전 계산해 메인 잠정 + QA 프롬프트에 입력 → QA의 중복 grep 제거(토큰↓), 매칭 일관성↑, 전수 대조로 누락 방지. (2026-06-01 lawear-985b 사용자 제안)

- **0a. 문자열 매칭 (파이썬 정규식, LLM 토큰 0)** — ⚠️case_id에 한글(모의고사·순 등) 포함 시 `urllib.parse.quote(case_id)`로 URL 인코딩 후 GET (urllib ascii 인코딩 에러 회피):
  - 조문 번호: 모범답안 `제\d+조(의\d+)?(\s*제?\d+항)?` 추출 → 사용자 답안과 매칭 → {매칭 / 누락 / 사용자 추가(원본 미언급)} 리스트.
  - 두문자: lv4 `[key]`/`[blank2]` 토큰 텍스트 → 사용자 답안 문자열 포함 여부.
- **0b. 의미 매칭 (Haiku/Sonnet 서브에이전트, Opus 1/10~1/20 비용)**:
  - color/under 키워드: lv4 `[red]`/`[u]` 키워드 → 사용자 답안 평문 동의어 매칭(표현 차이 무시, 의미 기반).
  - 조문 의미 추론: 사용자가 명시 안 했으나 사안 행위의 근거 조문 환기(예: '압수수색 시'→제215조).
  - 호출 예: `Task(model=haiku, subagent_type=general-purpose, prompt='모범답안 키워드 리스트 vs 답안 의미 매칭표만 생성. 점수·판단 X, 매칭 여부만.')`
- **산출 → 입력**: 매칭표(조문 X/O/추가 · 두문자 X/O · 키워드 매칭률)를 **2.3 메인 잠정 + 2.5 QA 프롬프트에 주입**(옵션 B 강화 — QA가 코드/모범답안 grep 불필요).

⚠️ **매칭됨 ≠ 정답 (필수 인지)**:
- 기계 매칭은 '댔는가'만 판정. **'맞는 조문인가(오인용)'·'동음이의 가짜 매칭'은 Opus(메인·QA) 판단 영역**.
- 사고: att98 규칙46 조문 매칭됐으나 오인용(정답 규칙105①) / [[feedback_em_markup_evidence]] 동음이의 가짜 매칭.
- 즉 **기계=추출(입력 자료) / Opus=정확성·의미 판단**. 매칭표를 점수로 직결 금지(특히 articles는 매칭 후 룰9 예외·오인용 검증).

**정책 정합**: lawear 'Opus만'([[feedback_no_subagent_for_board]])은 **판단 작업**(채점·QA·게시판) 한정 → 기계 추출(파이썬/Haiku)은 비판단 보조라 예외 (사용자 승인 2026-06-01).
...
6. **두문자 부재 문제 → mnem 평가 제외(N/A)** — 모범답안 Lv.4에 `[key]`/`[blank2]` 두문자가 없는 논점(수용·직권말소 등 라이브러리 미수록)은 **mnem을 채점에서 제외**(가점·감점 둘 다 없음, 존재하지 않는 두문자 미매칭 감점 금지 R-09). **계산: `max_score = 90`(100 − mnem 10), `score_pct = 나머지 8기준 점수 합 / 90 × 100`.** payload의 mnem criterion은 `score: 0` + `weight_applied: 0` + `comment: "N/A — 두문자 부재(평가 제외)"`로 기록. ☞ **두문자 있는 문제만 mnem 평가**(적으면 가점 / 안 적으면 감점). (장래효 2026-05-30, 사용자 결정. 소급 X.)
7. **부등서류·부동산등기서류(`subject=budeungseoryu`) → 시각 마크업 3기준 N/A + 6기준 재정의** — `mnem`·`color`·`under`(시각 강조 마크업)는 **채점 제외**(N/A, `weight_applied:0`). 등기신청서 작성 과목에서 형광펜/밑줄/두문자는 채점 항목이 아니며, 포함 시 점수를 왜곡(노이즈)함. **`max_score = 67`**(100 − mnem10 − color17 − under6), **`score_pct = 6기준 합 / 67 × 100`**. 6기준 의미 재정의: `outline`=등기신청서 양식 골격(표제/부동산표시/등기원인/등기목적/당사자/등기필정보/관할 귀중) / `sem`=양식 정확성(**등기목적·지분·당사자** 핵심 — 등기목적 오기·지분 오독은 권리변동 오공시로 치명 감점) / `articles`=첨부서면 근거 조문(규칙46·60·61, 법49·68, 민법42·45 등) / `rich`=첨부서면 적중·충실도 / `miss`=필수 첨부서면 누락(주무관청 허가서·거래계약신고필증·부동산등기용등록번호증명서·토지대장 등 **등기 진행 불가급 가중 차감**) / `case_apply`=사안 적용(종중 총유 사원총회결의·재단법인 기본재산 주무관청 허가·외국인 주소공증 등). ☞ 완전한 전용 3축 배점(양식 35 / 첨부 40 / 근거 25)은 server 가중치 지원 후 정밀화. (att95 부등서류 첫 채점, 사용자 결정 2026-05-30 lawear-985b)
8. **치명오류 cap (핵심 논점 오류/누락 시 상한)** — 부장판사·강사 QA가 식별한 **이 문제의 핵심 논점**(보통 ①핵심 결론 + ②그 문제 정체성 논점 1개)을 사용자가 **오류·누락**한 경우, 곁가지(조문·키워드) 점수가 누적돼도 합격되지 않도록 상한 적용: ▸ 핵심 **정체성 논점**(예: '변호인 참여권의 법적 성격=고유권', '판례 논거', '핵심 함정') 오류/누락 → 해당 기준(주로 `sem`, 사안 따라 `case_apply`) **≤ max × 0.5 cap**(예: sem 15→7) ▸ 핵심 **결론** 자체가 틀림 → **`score_pct ≤ 60` cap**(불합격 확정). **핵심 논점 식별은 QA 결과 연동 필수**(부장판사/강사가 "이 문제 핵심은 X"라 명시한 것만) — 메인 자의 cap 남발 금지, QA 근거 인용. 취지: 9기준 가산식이 '골격 맞고 핵심 틀린' 답안을 합격권으로 밀어올리는 후함 차단. (2026-06-01 lawear-985b 사용자 — att104 휴대전화 답안 76.67 A가 순수 주관 70 대비 후함, '고유권→헌법보장' 오귀속에 sem≤50% cap 적용 시 71.1% B+로 주관과 수렴)
9. **절차법(형소·민소·부등법) articles N/A 금지** — 형사소송법·민사소송법·부동산등기법은 **절차법 = 조문이 답안 골격**. `articles`를 N/A 처리하지 말 것(룰7 부등서류 전용 예외 외 전부 평가). 모범답안 Lv.4에 조문이 적게 적혔어도(판례 위주 서술) 그 사안의 근거 조문은 존재(예: 형소 압수수색 제215·219조·준항고 제417조 / 민소 관할·송달 / 부등 신청·첨부 조문)하므로 사용자가 댔는지 평가. 조문 0개 답안 → 룰2(articles cap 1점) 적용. **N/A 허용은 `mnem`만**(두문자 라이브러리 부재 시, 룰6) — 절차법 articles N/A는 '조문 안 쓴 것'을 면제해 점수를 부당 상향(후함 주범 = N/A 분모 축소). (2026-06-01 lawear-985b 사용자 '근거 없는 절차법은 말이 안 된다' — att103 형소 articles N/A 오판 76.47 A → 정정 73.33 A-)
   - **★예외 (판례 해석론 중심 — articles N/A 허용)**: '민소·형소·부등법이면 무조건 articles 평가'가 아니라 **'그 문제의 답안 골격이 절차조문 적시인가'로 판단**. (a)모범답안 조문 0개 + (b)정답 핵심이 판례 해석론 + (c)근거가 실체조문(민법404 등)이면 → articles N/A(분모서 제외, max95). 절차조문(압수수색 제215·219조·준항고 제417조 등)이 답안 골격일 때만 cap. 사례: att103 형소 압수수색=절차조문 골격→articles 평가(cap1) vs att106 민소 채권자대위 부존재 주장 가부=판례 해석론·민법404 실체조문·모범답안 조문0→articles N/A max95(부장판사·강사 QA 합치). 룰9 기계 적용(att106 잠정 articles1 cap)이 핵심판례 적중 가점을 침식한 사고 후 정밀화 (2026-06-01 lawear-985b).

**기준 의미**:
- **mnem (두문자 풀이형 매칭)**: Lv.4 [blank2]/[key] 두문자(예: 변/대/공/상)를 답안에 정확히 풀어 적었는지 매칭 비율. **두문자 있는 문제 한정**(적으면 가점 / 안 적으면 감점). **모범답안에 두문자가 없으면 평가 제외(N/A) → `max_score 90`으로 환산, mnem `weight_applied: 0` (grader v3 룰6 참조, 가점·감점 둘 다 없음).**
...
**★N/A 정규화 — QA 점수 체계 일치 (필수, 2026-06-01 lawear-985b)**:
- N/A 기준(mnem/articles 등)이 있으면 분모가 100→90/85 등으로 줄어든다. 이때 **QA가 점수를 100점 만점 답안 감각으로 주면(예: sem 13/15) 줄어든 분모로 나눠 % 가 부풀려진다**(N/A 분모축소 후함). N/A가 2개 이상이면 증폭.
- **QA 프롬프트에 명시**: "N/A 기준은 제외, 남은 기준 만점 합이 분모(예 85). 점수는 **남은 체계 기준**으로 매기되, **항목 점수 합이 너의 총점 직관과 일치**하게 하라"(항목↔총점 괴리 금지).
- **메인 검증**: QA 항목 합/분모 = % 가 QA 총점 직관과 어긋나면(특히 N/A 2개+) → 누락을 miss뿐 아니라 sem/rich/case에도 분산 반영해 **총점 직관에 정렬**.
- ⚠️ **분자를 N/A 비율로 추가 축소 금지(이중축소)** — N/A는 '평가대상 부재'지 '0점'이 아니므로, 분자×(85/100) 식으로 또 줄이면 사용자가 쓸 수도 없던 항목으로 부당 감점. `남은기준 점수합 / 남은기준 만점합`이 올바른 정규화.
- 사례: att107 QA 항목 70/85=82%(rich13·case10 후함) → 누락(소유권·가등기 유용) 반영 rich12·case8 → 67/85=**78.82%**(QA 직관 78 정렬).
