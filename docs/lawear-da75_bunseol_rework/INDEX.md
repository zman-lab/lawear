...
- **사용자 확인/검증 요청 시 모든 케이스 = 네비 찍어 보고** (영구 룰 — 모든 스킬 공통, sample N건 검증/의심 케이스/missing/split/오버분설/SPEC diff 등 전부). 트리 네비 경로 (`{년도}/{과목}/{순환}/{회차}/{제목}` 형식 — `feedback_navigation_path_not_filename` 메모리 룰 준수) + 본문 line + 변경 영역 발췌 명시. **단순 파일명 X, 사용자가 직접 17895/17896에서 찾아갈 수 있어야 함**.
